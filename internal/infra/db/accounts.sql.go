...
const updatedAccount = `-- name: UpdatedAccount :one
UPDATE accounts
SET balance = $2
WHERE id = $1
RETURNING id, owner, balance, currency, created_at
...
type UpdatedAccountParams struct {
	ID      int64 `json:"id"`
	Balance int64 `json:"balance"`
}

func (q *Queries) UpdatedAccount(ctx context.Context, arg UpdatedAccountParams) (Account, error) {
	row := q.db.QueryRowContext(ctx, updatedAccount, arg.ID, arg.Balance)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
